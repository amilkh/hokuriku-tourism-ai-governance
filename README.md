...
└── output/                       # Generated artifacts (created at run time, not committed)
```
...
All artifacts are written to `output/` (created at run time): paper figures, LaTeX tables, and `analysis_metrics.txt`.
...
Results are written to `output/analysis_metrics.txt` when the pipeline is run.
