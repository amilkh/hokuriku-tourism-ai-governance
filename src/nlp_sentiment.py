...
- Tokenization: fugashi MeCab → character n-grams
...
class Tokeniser:
    """
    Multilingual tokenizer: MeCab morphological segmentation → char n-grams.
...
class AspectSentimentAnalyzer:
    ...
    Aspect attribution via TF-IDF cosine similarity to description vectors.
    """

    def __init__(self, bert_device: int = -1) -> None:
    ...
    def fit(
        ...
        """Fit TF-IDF and sentiment model. Requires BERT or ≥30 ratings."""
        ...
        if self._classifier is None:
            ...
            valid_ratings = (
                ratings.dropna() if ratings is not None else pd.Series(dtype=float)
            )
            ...
            y_binary = (
                ratings.loc[valid_ratings.index] >= ratings.loc[valid_ratings.index].median()
            ).astype(int)
            self._lr_model = LogisticRegression(
                max_iter=1000, C=1.0, class_weight="balanced", random_state=42
            ).fit(X[valid_ratings.index], y_binary)
    ...
    def analyse_corpus(self, texts: pd.Series) -> pd.DataFrame:
        ...
        for i, text in enumerate(clean):
            row: dict = {"overall_sentiment": self.score_text(text)}
            row.update(self.aspect_scores(text, text_vec=X_all[i]))
            rows.append(row)
...
def run_nlp_analysis(
    ...
    rpt.metric("undervibrancy_ratio_nlp", round(uv_ratio, 2))
    ...
    rpt.metric("lda_num_topics",   topic_result.num_topics)
    rpt.metric("lda_score",        round(topic_result.score, 4))
    rpt.metric("lda_score_metric", topic_result.score_metric)
    ...
    if date_col in survey_df.columns:
        ...
        topic_ts  = (
            pd.DataFrame(
                topic_result.doc_topic_matrix, columns=topic_cols, index=dates
            )
            .groupby(lambda d: d.date())
            .mean()
        )
    ...
    if weather_df is not None:
        ...
        rpt.metric("sentiment_weather_r2", round(weather_corr.r_squared, 4))
    ...
    rpt.metric("top_negative_aspect", top_neg)
...
def nlp_to_latex(nlp_results: dict) -> str:
    """Generate a LaTeX table for paper inclusion (matches latex_export.py style)."""
    metrics   = nlp_results["summary_metrics"]
    aspect_df = (
        pd.DataFrame.from_dict(
            metrics["aspect_sentiments"], orient="index", columns=["Mean Sentiment"]
        )
        .sort_values("Mean Sentiment")
    )
    score_label = (
        "c\\_v coherence"
        if metrics.get("lda_score_metric") == "c_v_coherence"
        else "neg. perplexity"
    )
    lines = [
        r"\begin{table}[ht]",
        r"\centering",
        r"\caption{NLP Aspect Sentiment Analysis --- Hokuriku Survey Free-Text}",
        r"\label{tab:nlp_aspect_sentiment}",
        r"\begin{tabular}{lrr}",
        r"\toprule",
        r"Aspect & Mean Sentiment & Rank \\",
        r"\midrule",
    ]
    for rank, (asp, row) in enumerate(aspect_df.iterrows(), 1):
        lines.append(
            rf"{str(asp).replace('_', ' ').title()} & {row['Mean Sentiment']:+.4f} & {rank} \\"
        )
    lines += [
        r"\midrule",
        rf"\multicolumn{{2}}{{l}}{{Under-vibrancy ratio}} & "
        rf"{metrics['undervibrancy_ratio_nlp']:.1f}$\times$ \\",
        rf"\multicolumn{{2}}{{l}}{{LDA topics (k)}} & {metrics['lda_num_topics']} \\",
        rf"\multicolumn{{2}}{{l}}{{LDA {score_label}}} & {metrics['lda_score']:.4f} \\",
        rf"\multicolumn{{2}}{{l}}{{Sentiment model}} & {metrics['active_sentiment_model']} \\",
        r"\bottomrule",
        r"\end{tabular}",
        r"\end{table}",
    ]
