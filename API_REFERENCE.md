...
| `route_col` | `str` | RSI intent column name (used to tag intent-related features for ablation). |
...
| `ablation_table` | Random Forest on the **same split**: row `full` plus rows `no_weather`, `no_rsi_intent`, `no_calendar` when enabled in config.  Includes `delta_MAE_vs_full` and `delta_R2_vs_full`. |
...
- Feature families for ablation are defined in code: **weather** (`precip`, `temp`, `sun`, `wind`, `precip_lag1`, `weather_severity`, `weekend_x_severity`), **rsi_intent** (`route_col`, its `lag`/`roll` columns, `weekend_x_intent`), **calendar** (`is_weekend_or_holiday`, `month`, `dow_mean_count`).
